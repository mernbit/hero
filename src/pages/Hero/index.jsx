    ...
    return (
        ...
        >
            ...
            <div className="w-full min-h-screen lg:h-screen max-w-[1440px] mx-auto px-4 lg:px-12 py-4 lg:py-6 relative overflow-hidden">
                ...
                {/* ── Hero ── */}
                <section id='hero' ref={heroRef} className=" mt-20 lg:mt-14 grid grid-cols-1 lg:grid-cols-[260px_1fr_260px] grid-rows-auto lg:grid-rows-[auto_1fr] items-center lg:items-start min-h-[calc(100vh-120px)] relative gap-8 lg:gap-0 pb-10 lg:pb-0">
                    {/* Left */}
                    <div id='left' className="col-start-1 lg:col-start-1 lg:row-start-1 lg:row-end-3 self-center z-2 flex flex-col items-center lg:items-start text-center lg:text-left order-2 lg:order-1">
                        <h1 ref={headingRef} className="bungee-regular font-black text-[48px] lg:text-[64px] leading-none mb-4 text-text-main">{activeFlavor.name}</h1>
                    ...
                    {/* Center */}
                    <div id='img' className="col-start-1 lg:col-start-2 lg:row-start-1 lg:row-end-3 flex items-center justify-center z-1 relative order-1 lg:order-2 h-[320px] lg:h-auto mt-4 lg:mt-0">
                        {/* Particles */}
                        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-[-1]">
                            {Array(7).fill(0).map((_, i) => (
                                <img key={i} ref={el => particleRefs.current[i] = el} src={activeFlavor.particle} alt="" className="absolute w-20 h-20 lg:w-30 lg:h-30 object-contain opacity-0 scale-50" />
                            ))}
                        </div>

                        <img ref={coneImgRef} src={activeFlavor.cone} alt={`${activeFlavor.name} ice cream cone`} className="h-[280px] lg:h-156 rotate-z-335 max-w-full w-auto object-contain lg:-mt-5 drop-shadow-2xl" />
                    </div>
                    ...
                    <div className="col-start-1 lg:col-start-3 lg:row-start-1 lg:row-end-3 flex flex-col items-center lg:items-end gap-8 lg:gap-10 lg:pt-2.5 order-3">
                        ...
                        <div id='flavors' className="flex flex-row lg:flex-col overflow-x-auto lg:overflow-visible w-full lg:w-auto py-2 lg:py-0 gap-3 lg:gap-1.5 lg:mt-auto">
                            {flavors.map((f) => {
                                ...
                                return (
                                    <div key={f.name} onClick={() => handleFlavorClick(f)} className={`flex items-center gap-[10px] lg:gap-[14px] rounded-full py-2 lg:py-3 pr-6 lg:pr-14 pl-2 min-w-[140px] lg:min-w-[180px] cursor-pointer border-2 transition-all shrink-0 hover:scale-105 ${isActive ? 'border-lavender bg-white shadow-md' : 'border-transparent bg-bg-card hover:bg-gray-100'}`}>
                                        <img src={f.img} alt={f.name} className="w-10 h-10 lg:w-12 lg:h-12 rounded-full object-cover shadow-sm" />
                                        <span className="text-[14px] lg:text-[15px] font-semibold">{f.name}</span>
                                    </div>
                                )
                            })}
                        </div>
