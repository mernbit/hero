        ...
        <footer className="w-full bg-white">
                ...
                <div className="grid grid-cols-1 md:grid-cols-[auto_1fr_auto] gap-8 md:gap-16 items-start">
                    ...
                    <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
                        ...
                        <div className="w-[100px] h-[100px] rounded-full bg-accent flex items-center justify-center shrink-0">
                            <svg viewBox="0 0 40 40" fill="none" className="w-[50px] h-[50px]">
                        ...
                        <div className="flex flex-col gap-2 text-center sm:text-left">
                            <h3 className="font-bold text-[18px] text-text-main">{tenant.name}</h3>
                            <p className="text-[14px] text-text-muted">
                                <span className="font-semibold text-text-main">Phone: </span>{footer.phone}
                            </p>
                            <p className="text-[14px] text-text-muted">
                                <span className="font-semibold text-text-main">Email: </span>{footer.email}
                            </p>
                            <p className="text-[14px] text-text-muted">
                                <span className="font-semibold text-text-main">Address: </span>{footer.address}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center md:items-start gap-4">
                        ...
                        <div className="flex gap-3">
                            ...
                            <a href={footer.socials.instagram} target="_blank" rel="noopener noreferrer" 
                               className="w-[36px] h-[36px] rounded-full bg-gradient-to-tr from-[#f09433] via-[#dc2743] to-[#bc1888] flex items-center justify-center hover:opacity-80 transition-opacity">
                                <svg viewBox="0 0 24 24" fill="#fff" className="w-[18px] h-[18px]">
                        ...
                        <div className="flex gap-6 mt-2">
                            {footer.links.map((link, i) => (
                                <a key={i} href="#" className="text-[14px] text-text-main underline underline-offset-2 hover:text-accent transition-colors">
                                    {link}
                                </a>
                            ))}
                        </div>
            ...
            <div className="border-t border-gray-200 py-4">
                <div className="max-w-[1440px] mx-auto px-4 md:px-12 flex items-center justify-center gap-2">
                    <span className="text-[13px] text-text-muted">© {new Date().getFullYear()} Powered by</span>
                    <span className="font-black text-[18px] text-text-main tracking-tight" style={{ fontFamily: "'Inter', sans-serif" }}>{footer.poweredBy}</span>
                </div>
            </div>
        </footer>
    );
